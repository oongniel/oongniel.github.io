    ...
    function bindEvents(MQ, bool) {
    	...
    	if( MQ == 'desktop' && bool) {   		
    		...
    		$(document).on('keydown', function(event){
				...
				if( (event.which=='40' || event.which=='39') && !nextArrow.hasClass('inactive') ) {
					event.preventDefault();
					nextSection();
					$('body').trigger('page-change');
				} else if( (event.which=='38' || event.which=='37') && (!prevArrow.hasClass('inactive') || (prevArrow.hasClass('inactive') && $(window).scrollTop() != sectionsAvailable.eq(0).offset().top) ) ) {
					event.preventDefault();
					prevSection();
					$('body').trigger('page-change');
				} else {
